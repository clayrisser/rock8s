...
get_worker_output_json() {
    ...
    if [ -f "$_WORKER_OUTPUT_JSON_FILE" ]; then
        ...
        fail "worker output.json not found"
    fi
    echo "$_WORKER_OUTPUT_JSON"
}
...
get_worker_node_count() {
    ...
    _WORKER_NODE_COUNT="$(get_worker_output_json | jq -r '.node_private_ipv4s.value | length')"
    echo "$_WORKER_NODE_COUNT"
}
...
get_worker_private_ipv4s() {
    ...
    _WORKER_PRIVATE_IPV4S="$(get_worker_output_json | jq -r '.node_private_ipv4s.value | .[] | @text')"
    echo "$_WORKER_PRIVATE_IPV4S"
}
...
get_worker_public_ipv4s() {
    ...
    _WORKER_PUBLIC_IPV4S="$(get_worker_output_json | jq -r '.node_public_ipv4s.value | .[] | @text')"
    echo "$_WORKER_PUBLIC_IPV4S"
}
