...
get_node_arch() {
    ...
    ssh_key="$2"
    ip="$3"
    output_file="$(get_cluster_dir)/${TF_VAR_purpose:-master}/output.json"
    arch=""
    if [ -f "$output_file" ]; then
        arch="$(jq -r --arg name "$node_name" '.node_architectures.value[$name] // ""' < "$output_file" 2>/dev/null)"
    fi
    if [ -z "$arch" ]; then
        uname_out="$(ssh -o StrictHostKeyChecking=no -o ConnectTimeout=10 -i "$ssh_key" admin@"$ip" 'uname -m' 2>/dev/null)" || true
        case "$uname_out" in
            x86_64)  arch="amd64" ;;
            aarch64) arch="arm64" ;;
            *)       arch="amd64" ;;
...
get_k3s_install_env() {
    arch="$1"
    if [ "$arch" = "arm64" ]; then
        echo "INSTALL_K3S_EXEC= INSTALL_K3S_CHANNEL=stable"
    else
        echo ""
    fi
}
