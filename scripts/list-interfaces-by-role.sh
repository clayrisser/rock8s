...
echo "$_UPLINK=uplink:$($interface_data | grep -E "^$_UPLINK=" | sed "s|^$_UPLINK=||g")"
echo "$_CEPH=ceph:$($interface_data | grep -E "^$_CEPH=" | sed "s|^$_CEPH=||g")"
echo "$_PRIVATE=private:$($interface_data | grep -E "^$_PRIVATE=" | sed "s|^$_PRIVATE=||g")"
