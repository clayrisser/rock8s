...
echo "$_UPLINK=uplink:$($_INTERFACE_DATA | grep -E "^$_UPLINK=" | sed "s|^$_UPLINK=||g")"
echo "$_CEPH=ceph:$($_INTERFACE_DATA | grep -E "^$_CEPH=" | sed "s|^$_CEPH=||g")"
echo "$_PRIVATE=private:$($_INTERFACE_DATA | grep -E "^$_PRIVATE=" | sed "s|^$_PRIVATE=||g")"
