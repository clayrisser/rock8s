...
iface vmbr$(echo $_VLAN_ID | sed 's|^40||') inet static
    ...
    post-up      iptables -t nat -A PREROUTING -i vmbr0 -p tcp -d $PUBLIC_IP_ADDRESS -j DNAT --to $(echo $GUEST_SUBNET | sed "s|^\(.*\)\.\([0-9]\)*\/\([0-9]*\)$|\1.$HOST_NUMBER|g")
    post-down    iptables -t nat -D PREROUTING -i vmbr0 -p tcp -d $PUBLIC_IP_ADDRESS -j DNAT --to $(echo $GUEST_SUBNET | sed "s|^\(.*\)\.\([0-9]\)*\/\([0-9]*\)$|\1.$HOST_NUMBER|g")
