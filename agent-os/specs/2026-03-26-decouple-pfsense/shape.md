...
- **`rock8s pfsense` stays** — it provisions pfSense VMs using the same OpenTofu provider, configures them with Ansible (PHP scripts modifying XML), and supports HA (primary + secondary). But it operates independently of any cluster.
...
- **`pfsense` purpose removed from cluster OpenTofu** — the provider module only handles `master` and `worker`. Network and route are always `data` lookups (must already exist).
