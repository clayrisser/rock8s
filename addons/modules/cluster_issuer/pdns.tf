resource "kubernetes_cluster_role_v1" "cert_manager_webhook_pdns_configmap_reader" {
...
resource "kubernetes_cluster_role_binding_v1" "cert_manager_webhook_pdns_configmap_reader" {
  count = (var.enabled && try(var.issuers.pdns.api_url != "" && var.issuers.pdns.api_key != "", false)) ? 1 : 0
  metadata {
    name = "cert-manager-webhook-pdns-configmap-reader"
  }
...
EOF
  ...
  depends_on = [
    kubernetes_cluster_role_v1.cert_manager_webhook_pdns_configmap_reader,
    kubernetes_cluster_role_binding_v1.cert_manager_webhook_pdns_configmap_reader
  ]
}
