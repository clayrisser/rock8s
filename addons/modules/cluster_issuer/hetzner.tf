resource "kubernetes_cluster_role_v1" "cert_manager_webhook_hetzner_configmap_reader" {
...
resource "kubernetes_cluster_role_binding_v1" "cert_manager_webhook_hetzner_configmap_reader" {
  count = (var.enabled && try(var.issuers.hetzner.api_key != "", false)) ? 1 : 0
  metadata {
    name = "cert-manager-webhook-hetzner-configmap-reader"
  }
...
resource "helm_release" "cert-manager-webhook-hetzner" {
  ...
  version          = var.hetzner_webhook_version
  namespace        = var.namespace
  create_namespace = true
  values           = []
  depends_on = [
    kubernetes_cluster_role_v1.cert_manager_webhook_hetzner_configmap_reader,
    kubernetes_cluster_role_binding_v1.cert_manager_webhook_hetzner_configmap_reader
  ]
}
...
EOF
  depends_on = [
    ...
    helm_release.cert-manager-webhook-hetzner
  ]
}
