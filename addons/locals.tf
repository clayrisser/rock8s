locals {
  ...
  load_balancer      = var.ingress_nginx ? try(data.kubernetes_service.ingress_nginx[0].status[0].load_balancer[0].ingress[0].ip, null) : null
  ...
  email              = var.email != null && var.email != "" ? var.email : var.cloudflare_email
  ...
  s3                 = var.s3 && var.integration_operator
}
